...
const count = (target, input) => {
  ...
  input.map((i) => {
    i.map((x) => {
      if (x === target) { countedArr.push(1); }
    });
  });
  return countedArr.length;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(inputArr =>
    inputArr.filter(x => ((typeof x) === 'number') && (x % 5 === 0))
      .map(x => Math.pow(2, x)));
};
...
let findMaleAndFemale = (data) => {
  ...
  (item.gender === 'female')).map(item => item.name).join(' and ');
};
...
let findShortest = (data) => {
  ...
  Number(first.height) < Number(second.height) ? first : second).name;
};
