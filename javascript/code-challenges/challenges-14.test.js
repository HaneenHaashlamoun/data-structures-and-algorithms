...
const screenForNames = (arr) => {
  // Solution code here...
  let names = /^(Mr\.|Mrs\.|Ms\.|Dr\.).\w*$/;
  return arr.filter((index) => index.match(names));
}
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    if (first[0] == second[0] == third[0] || first[0] == second[1] == third[2] || first[2] == second[1] == third[0] || first[0] == first[1] == first[2] || second[0] == second[1] == second[2] || third[0] == third[1] == third[2] || first[1] == second[1] == third[1] || first[2] == second[2] == third[2]) {
      return true
    } else {
      return false
    }
